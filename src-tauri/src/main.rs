...
#[tauri::command]
fn cmd_set_start(secs: u32, tx: tauri::State<mpsc::SyncSender<TickerControl>>) {
    tx.send(TickerControl::Start(secs)).unwrap();
}
    ...
    let app = app.on_system_tray_event(move |app, event| match event {
            ...
            SystemTrayEvent::MenuItemClick { id, .. } => {
                match id.as_str() {
                    ...
                    "About" => {
                        let window = app.get_window("main").unwrap();
                        tauri::api::dialog::message(Some(&window), "sstimer", about);
                    }
                    "Licenses" => {
